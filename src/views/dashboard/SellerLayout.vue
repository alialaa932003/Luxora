...
const navItems = [
  { label: 'Overview', to: '/seller/overview', icon: LayoutDashboard },
  { label: 'Products', to: '/seller/products', icon: Package },
  { label: 'Orders', to: '/seller/orders', icon: ShoppingBag },
]

const storeName = computed(() => {
  if (authStore.user?.id === 'vendor-1') return 'StyleVault'
  return 'My Store'
})
...
function getInitials(name: string) {
  return name?.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() ?? 'SL'
}
    ...
    >
      ...
      <div class="flex items-center gap-3 px-6 py-5" style="border-bottom: 1px solid oklch(0.88 0.008 85);">
        ...
        <div>
          ...
          <p class="text-xs font-medium" style="color: oklch(0.52 0.015 285);">Seller Panel</p>
        </div>
      ...
      <div style="border-top: 1px solid oklch(0.88 0.008 85);" class="px-3 py-3 space-y-1">
        <!-- Switch to Admin Panel (Visible to Admins) -->
        <RouterLink
          v-if="authStore.isAdmin"
          to="/admin"
          class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150"
          style="color: oklch(0.51 0.22 291 / 0.85); background: oklch(0.51 0.22 291 / 0.04);"
        >
          <ShieldCheck :size="16" />
          Switch to Admin
        </RouterLink>
        ...
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-full overflow-hidden flex-shrink-0">
            ...
            <div v-else class="w-full h-full flex items-center justify-center text-white text-xs font-bold"
              style="background: oklch(0.51 0.22 291);">
              {{ getInitials(`${authStore.user?.firstName ?? 'S'} ${authStore.user?.lastName ?? 'O'}`) }}
            </div>
          </div>
          <div class="min-w-0 flex-1">
            ...
            <p class="text-xs truncate" style="color: oklch(0.52 0.015 285);">Seller Account</p>
          </div>
        </div>
      </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-h-screen lg:pl-[260px]">
      ...
      <header class="sticky top-0 z-10 flex items-center gap-4 px-6 py-4"
        ...
        <div class="flex items-center gap-2 text-sm">
          <span style="color: oklch(0.52 0.015 285);">Seller</span>
