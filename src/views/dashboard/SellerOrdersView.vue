...
import { Search, ChevronLeft, ChevronRight, ShoppingBag } from 'lucide-vue-next'
...
async function fetchOrders(showSpinner = true) {
  ...
  try {
    // Note: The sellerService.getOrders mock automatically partitions orders for the current vendor
    result.value = await sellerService.getOrders(vendorId.value, {
      page: page.value,
      limit,
      search: search.value,
    } as any)
...
<template>
  <div class="space-y-6 animate-fade-in">
    <!-- Header -->
    <div>
      <h1 class="text-2xl font-bold tracking-tight" style="color: oklch(0.14 0.02 280);">My Orders</h1>
      <p class="text-sm mt-0.5" style="color: oklch(0.52 0.015 285);">
        Manage customer orders for your products · <span class="font-semibold">{{ result?.total ?? 0 }} total</span>
      </p>
    </div>
    ...
    <div class="rounded-2xl border overflow-hidden bg-white" style="border-color: oklch(0.88 0.008 85);">
      ...
      <div v-else-if="!result?.data.length" class="text-center py-20">
        ...
        <p class="text-sm mt-1" style="color: oklch(0.52 0.015 285);">Orders will show up here once purchased</p>
      </div>
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody>
            <tr v-for="order in result?.data" :key="order.id" style="border-bottom: 1px solid oklch(0.93 0.004 85);">
              <!-- Order # -->
              <td class="px-6 py-4">
